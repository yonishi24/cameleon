 ...
 * file, You can obtain one at https://mozilla.org/MPL/2.0/. */
 ...
 pub(crate) struct GEVDeviceModule {
     vm: genapi::Memory,
     port_info: PortInfo,
     xml_infos: Vec<XmlInfo>,
 
     camera: Camera,
     remote_device: Option<Box<Mutex<GEVRemoteDevice>>>,
 ...
 impl GEVDeviceModule {
     ...
     pub(crate) fn device_info(&self) -> &gev::DeviceInfo {
         todo!()
     }
     ...
     /// Make sure to call [`GEVDeviceModule::reflect_status`] to obtain up to date status before
     /// calling [`GEVDeviceModule::access_status`].  
     /// See GenTL specification for more details.
 ...
 impl Drop for GEVDeviceModule {
     fn drop(&mut self) {
         self.close().ok();
     }
 }
 
 impl Port for GEVDeviceModule {
 ...
 impl Device for GEVDeviceModule {
 ...
 pub(crate) struct GEVRemoteDevice {}
 
 impl GEVRemoteDevice {
     fn new(handle: gev::ControlHandle) -> GenTlResult<Self> {
         todo!()
     }
 
     fn port_info(handle: &gev::ControlHandle) -> GenTlResult<PortInfo> {
         todo!()
     }
 
     fn xml_infos(handle: &gev::ControlHandle) -> GenTlResult<Vec<XmlInfo>> {
         todo!()
     }
 }
 
 impl Port for GEVRemoteDevice {
     fn read(&self, address: u64, buf: &mut [u8]) -> GenTlResult<usize> {
         todo!()
     }
