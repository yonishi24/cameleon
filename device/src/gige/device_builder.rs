...
use crate::gev::{BusSpeed, DeviceInfo, Error, Result};
...
impl DeviceBuilder {
    fn new(device: RusbDevice) -> Result<Option<Self>> {
        ...
        {
            if let Some((iad, conf_desc)) = Self::find_gev_iad(&device, &device_desc)? {
                return Ok(Some(Self {
                    device,
                    gev_iad: iad,
    ...
    fn build(self) -> Result<Device> {
        ...
        let mut interfaces = self
            .config_desc
            .interfaces()
            .skip_while(|iface| iface.number() != self.gev_iad.first_interface);
    ...
    fn find_gev_iad(
        ...
        for config_index in 0..num_config_desc {
            ...
            if let Some(gev_iad) = Self::find_gev_iad_in_config_desc(&config_desc) {
                return Ok(Some((gev_iad, config_desc)));
            }
        }
    ...
    fn find_gev_iad_in_config_desc(desc: &rusb::ConfigDescriptor) -> Option<Iad> {
        if let Some(iad) = Iad::from_bytes(desc.extra()) {
            if Self::is_gev_iad(&iad) {
                return Some(iad);
            }
        }
        ...
        for iface in desc.interfaces() {
            for if_desc in iface.descriptors() {
                if let Some(gev_iad) = Self::find_gev_iad_in_if_desc(&if_desc) {
                    return Some(gev_iad);
                }
            }
        }
    ...
    fn find_gev_iad_in_if_desc(desc: &rusb::InterfaceDescriptor) -> Option<Iad> {
        if let Some(iad) = Iad::from_bytes(desc.extra()) {
            if Self::is_gev_iad(&iad) {
                return Some(iad);
            }
        ...
        for ep_desc in desc.endpoint_descriptors() {
            if let Some(gev_iad) = Self::find_gev_iad_in_ep_desc(&ep_desc) {
                return Some(gev_iad);
            }
        }
    ...
    fn find_gev_iad_in_ep_desc(desc: &rusb::EndpointDescriptor) -> Option<Iad> {
...
struct DeviceInfoDescriptor {
    ...
    gev_version_major: u16,
    gev_version_minor: u16,
    guid_idx: u8,
    vendor_name_idx: u8,
    model_name_idx: u8,
    family_name_idx: u8,
...
impl DeviceInfoDescriptor {
    ...
    fn from_bytes(mut bytes: &[u8]) -> Result<Self> {
        ...
        let gev_version_minor = bytes.read_bytes_le()?;
        let gev_version_major = bytes.read_bytes_le()?;
    ...
    fn interpret(&self, channel: &RusbDeviceHandle) -> Result<DeviceInfo> {
        ...
        let gev_version = Version::new(
            self.gev_version_major.into(),
            self.gev_version_minor.into(),
            0,
        );
