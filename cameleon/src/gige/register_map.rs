...
impl Sbrm {
    ...
    pub fn new<Ctrl: DeviceControl + ?Sized>(
        ...
        let (capability_offset, capability_len) = sbrm::GEVCP_CAPABILITY_REGISTER;
    ...
    /// Version of GEV of the device.
    pub fn gev_version<Ctrl: DeviceControl + ?Sized>(
        &self,
        device: &mut Ctrl,
    ) -> ControlResult<semver::Version> {
        let gev_version: u32 = self.read_register(device, sbrm::GEV_VERSION)?;
        let gev_version_minor = gev_version & 0xff;
        let gev_version_major = (gev_version >> 16_i32) & 0xff;

        Ok(semver::Version::new(
            u64::from(gev_version_major),
            u64::from(gev_version_minor),
            0,
        ))
    }
    ...
    ///
    /// NOTE: Some device doesn't support this feature.
    /// Please refer to [`GEVCapablitiy`] to see whether the feature is available on the device.
...
/// Indicate some optional GEV specific features are supported or not.
#[derive(Clone, Copy, Debug)]
pub struct GEVCapablitiy(u64);
...
impl GenICamFileInfo {
    ...
    pub fn file_type(&self) -> ControlResult<GenICamFileType> {
        ...
        match raw {
            ...
            _ => Err(ControlError::InvalidDevice(
                format!("Invalid GEV GenICamFileType value: {}", raw).into(),
            )),
        }
    }
    ...
    pub fn compression_type(&self) -> ControlResult<CompressionType> {
        ...
        match raw {
            ...
            _ => Err(ControlError::InvalidDevice(
                format!("Invalid GEV GenICamFilFormat value: {}", raw).into(),
            )),
        }
    }
...
impl ParseBytes for GEVCapablitiy {
    fn parse_bytes(bytes: &[u8]) -> ControlResult<Self> {
        Ok(Self(u64::parse_bytes(bytes)?))
    }
}

impl ParseBytes for gev::BusSpeed {
    fn parse_bytes(bytes: &[u8]) -> ControlResult<Self> {
        use gev::BusSpeed::{FullSpeed, HighSpeed, LowSpeed, SuperSpeed, SuperSpeedPlus};
