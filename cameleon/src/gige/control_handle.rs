...
//! This module contains low level device control implementation for `GEV` device.
...
/// convenient way to communicate with `gev` specific registers.
///
/// # Examples
///
/// ```no_run
...
impl ControlHandle {
    ...
    pub fn device_info(&self) -> &gev::DeviceInfo {
        &self.info
    }
    ...
    pub fn sirm(&mut self) -> ControlResult<Sirm> {
        ...
        let addr = self.sbrm()?.sirm_address(self)?.ok_or_else(|| {
            ControlError::InvalidDevice("the gev device doesn't have `SIRM ADDRESS`".into())
        })?;
    ...
    pub(super) fn new(device: &gev::Device) -> ControlResult<Self> {
